<?php
  /**
    ...
    $upload->set(array('path'=>'../../../File/outsource/','allowtype' = array('jpg','gif','png')))->upload();
    返回信息 Array：['success'=>true为成功，false为失败,'msg'=>报错信息,'filename'=>保存的文件名，多文件则为包含所有文件名的数组];
  */
class FileUpload
{

    private $path = "../../upload";          //默认为tyoasystem下的upload
    private $allowtype = array('jpg','gif','png','doc','docx','xls','xlsx','pdf'); //设置限制上传文件的类型
    ...
    function upload($fileField='file')
    {
        $return = true;
      /* 检查文件路径是否合法 */
        if (!$this->checkFilePath()) {
            $this->errorMess = $this->getError();
            return array('success'=>false,'msg'=>$this->errorMess,'filename'=>$this->newFileName);
        }
      /* 将文件上传的信息取出赋给变量 */
        $name = $_FILES[$fileField]['name']; //上传文件的文件名 
        $tmp_name = $_FILES[$fileField]['tmp_name']; //服务器端临时文件存放路径 
        $size = $_FILES[$fileField]['size']; //上传文件的大小 
          ...
          /*上传单个文件处理方法*/
            ...
            if ($this->setFiles($name, $tmp_name, $size, $error)) {
                ...
                if ($this->checkFileSize() && $this->checkFileType()) {
                    ...
                    /* 上传文件  返回0为成功， 小于0都为错误 */
                    if ($this->copyFile()) {
                        return array('success'=>$return,'msg'=>$this->errorMess,'filename'=>$this->newFileName);
                    } else {
                        $return=false;
                    }
                } else {
    ...
    private function copyFile()
    {
        if (!$this->errorNum) {
            ...
            $path = iconv("UTF-8", "GBK//IGNORE",$path);//文件名包含中文
